...
- **뷰-게이팅(실측 반영):** 지표마다 '사는 축'이 있고 그 축이 카메라 화면 안이면 신뢰·깊이축이면 불가. **스웨이(VF031/034)는 정면(FO) 전용** — 측면(DTL)에선 타깃선이 깊이축이라 측정 불가(실측 확인) → **`value:null` + `error_flags:["view_mismatch"]`** 로 정직하게. 팔·무릎 세그먼트각은 3D라 양쪽 뷰 OK. 뷰 unknown이면 계산하되 `off_axis_view` 감점.
