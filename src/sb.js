...
const NS = (() => {
  ...
  async function profiles() {
    ...
    await ready();
    if (!c) return [];
    const r = await c.from('profiles').select('id,nickname,real_name,is_pro,plan,created_at,research');
    return r.error ? [] : (r.data || []);
  }

  /* 수요조사 답 — 프로가 CRM 조사 칸에서 적는다. 정식 오픈의 플랜·가격을
     정할 답이라, 카톡에 흩어지지 않고 회원 옆에 붙어 있어야 한다. */
  async function setResearch(id, obj) {
    const c = client();
    await ready();
    const r = await c.from('profiles').update({ research: obj }).eq('id', id);
    if (r.error) throw r.error;
  }
  ...
  return {
    ...
    setName, setPlan, setResearch, note, want, wipe,
