...
SRC = os.path.join(HERE, 'beta-board.html')
...
# ── 스윙분석 엔진 원본을 그대로 싣는다 (수정 금지 · 복사만) ──
swcss = open(os.path.join(HERE, 'swbuild', 'components', 'swing', 'swing.css'), encoding='utf-8').read()
swjs  = open(os.path.join(HERE, 'swbuild', 'swing.bundle.js'), encoding='utf-8').read()
out = out.replace('</style>', swcss + '\n</style>', 1)
out = out.replace('<script>\ndocument.getElementById(\'openSw\')',
                  '<script>\n' + swjs + '\n</script>\n<script>\ndocument.getElementById(\'openSw\')', 1)

open(os.path.join(HERE, 'beta-board.built.html'), 'w', encoding='utf-8').write(out)
