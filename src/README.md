...
2. **문장은 프로가 고른다.** 기계는 주제어가 든 문장 둘을 미리 체크할 뿐이고,
   빼고 더하는 건 사람이 한다.
2.5 **작업대는 도착함과 같은 것을 쓴다** — 그 달 스윙을 골라 열고, 확대·선 긋기·
   캡처가 그대로 된다(`wbVideo()` 를 두 화면이 나눠 쓴다). 캡처한 사진은
   `reviews.photos` 로 요약에 같이 실린다.
...
### AI 초안 — 앱이 직접 부르지 않는 이유

「물어볼 글 복사하기」가 이번 달 한마디 전부를 **완성된 프롬프트**로 만들어
클립보드에 담는다. ChatGPT·클로드에 붙여넣고 나온 초안을 **고쳐서** 쓴다.

앱이 AI 를 직접 부르지 않는 건 **브라우저에 넣는 열쇠는 누구나 꺼내 쓸 수
있기 때문**이다. 지금 구조(정적 HTML + Supabase)에는 열쇠를 숨길 자리가 없다.
서버(Edge Function)가 생기면 그때 앱 안에서 부르게 바꾼다 — 그때까지는
프롬프트 품질을 앱이 책임지고, 부르는 건 사람이 한다.
