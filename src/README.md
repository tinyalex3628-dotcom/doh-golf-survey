...
`../design/` 에 있다. 아티팩트(claude.ai 페이지)로 만든 것이라 저장소 밖에
두면 다음 세션이 못 찾아서, HTML 째로 같이 넣어 뒀다.

- `design/opening-screen.html` — **여는 화면 · 오늘의 한 장.**
  머리는 고정이고 가운데 한 장만 매번 바뀐다. 카드 스물둘 · 갈래 아홉,
  「얼마 만에 왔나」가 순서를 정한다. 자세한 건 `design/README.md`.
